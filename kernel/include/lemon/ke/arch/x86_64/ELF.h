...
struct ELFData {
    ...
    Vector<ELFDynamicEntry> dynamic;
    Vector<ELFProgramHeader> programHeaders;
    Vector<uint8_t*> dataSegments;

    char linkerPath[PATH_MAX];
};

int VerifyELF(void* elf);
[[nodiscard]] Error elf_load_file(const FancyRefPtr<File>& file, ELFData& info);
[[nodiscard]] Error elf_load_segments(Process* proc, ELFData& data, uintptr_t base);
