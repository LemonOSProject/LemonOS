...
qemunetdump(){
	qemu-system-x86_64 --enable-kvm $LEMOND/Disks/Lemon.vhd -no-reboot -no-shutdown -m 512M -device qemu-xhci -M q35 -smp 2 -serial stdio -netdev user,id=net0 -device e1000,netdev=net0,mac=DE:AD:69:BE:EF:42 -object filter-dump,id=net0,netdev=net0,file=/tmp/lemon.pcap
}
...
debug_qemu(){
	...
	qemu-system-x86_64 -s -S Disks/Lemon.vhd -no-reboot -no-shutdown --enable-kvm -chardev stdio,id=gdb0 -device isa-debugcon,iobase=0x402,chardev=gdb0,id=d1 -d int -m 512M -M q35 -smp 1 -netdev user,id=net0 -device e1000,netdev=net0,mac=DE:AD:69:BE:EF:42&
