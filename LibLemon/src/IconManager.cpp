...
namespace Lemon {
    ...
    const Surface* IconManager::GetIcon(const std::string& name, IconSize preferredSize){
        ...
        if(preferredSize == IconSize16x16){
            ...
            if(!Lemon::Graphics::LoadImage((std::string("/system/lemon/resources/icons/16/") + name + ".png").c_str(),
                0, 0, 16, 16, &icon.icon16, false)) {
                return &icon.icon16;
            }

            if(!Lemon::Graphics::LoadImage((std::string("/system/lemon/resources/icons/32/") + name + ".png").c_str(),
                0, 0, 16, 16, &icon.icon16, false)) {
                return &icon.icon16;
            }

            if(!Lemon::Graphics::LoadImage((std::string("/system/lemon/resources/icons/64/") + name + ".png").c_str(),
                0, 0, 16, 16, &icon.icon16, false)) {
                return &icon.icon16;
            }
