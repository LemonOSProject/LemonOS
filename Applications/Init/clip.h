...
void SplitRect(rect_t victim, rect_t cutter, List<rect_t>* rects){
    ...
    if(GET_LEFT(cutter) >= GET_LEFT(victim) && GET_LEFT(cutter) <= GET_RIGHT(victim)){
        rects->add_back({victim.pos, {(cutter.pos.x - victim.pos.x) - 1, victim.size.y}}); // Split rect vertically on the left
    ...
    if(GET_RIGHT(cutter) >= GET_LEFT(victim) && GET_RIGHT(cutter) <= GET_RIGHT(victim)){
        //rects->add_back({victim.pos, {(GET_RIGHT(cutter) - victim.pos.x) + 1, victim.size.y}}); // Split rect vertically on the left
    ...
    /*if(GET_BOTTOM(cutter) >= GET_TOP(victim) && GET_BOTTOM(cutter) <= GET_BOTTOM(victim)){
        rects->add_back({victim.pos, {victim.size.x, (GET_BOTTOM(cutter) - victim.pos.y) + 1}}); // Split rect vertically on the left
