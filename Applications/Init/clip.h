...
void SplitRect(rect_t victim, rect_t cutter, std::vector<rect_t>* rects){

    if(GET_LEFT(cutter) > GET_LEFT(victim) && GET_LEFT(cutter) <= GET_RIGHT(victim)){
        rects->push_back({victim.pos, {(GET_LEFT(cutter) - GET_LEFT(victim)) - 1, victim.size.y}}); // Split rect vertically on the left
    ...
    if(GET_TOP(cutter) > GET_TOP(victim) && GET_TOP(cutter) <= GET_BOTTOM(victim)){
        rects->push_back({victim.pos, {victim.size.x, (cutter.pos.y - victim.pos.y) - 1}}); // Split rect vertically on the left
    ...
    if(GET_RIGHT(cutter) >= GET_LEFT(victim) && GET_RIGHT(cutter) < GET_RIGHT(victim)){
        rects->push_back({{GET_RIGHT(cutter) + 1, victim.pos.y},{GET_RIGHT(victim) - (GET_RIGHT(cutter) + 1), victim.size.y}});

        victim.size.x = (GET_RIGHT(cutter) - victim.pos.x);
    }
    
    if(GET_BOTTOM(cutter) >= GET_TOP(victim) && GET_BOTTOM(cutter) < GET_BOTTOM(victim)){
        rects->push_back({{victim.pos.x, GET_BOTTOM(cutter) + 1}, {victim.size.x, GET_BOTTOM(victim) - (GET_BOTTOM(cutter) + 1)}}); // Split rect vertically on the left
