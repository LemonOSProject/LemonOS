...
void Scroll() {
    ...
    while (static_cast<std::size_t>(bufferOffset + rowCount) >= buffer.size())
...
void OnPaint(surface_t* surface) {
    ...
    if (paintAll) {
        for (int i = 0; i < rowCount && (bufferOffset + i) < static_cast<int>(buffer.size()); i++) {
            ...
            Lemon::Graphics::DrawRect(j * 8, i * fontHeight, window->GetSize().x - j * 8, fontHeight,
                                      colours[state.bgColour], surface);
        }
        ...
        Lemon::Graphics::DrawRect(j * 8, curPos.y * fontHeight, window->GetSize().x - j * 8, fontHeight,
                                  colours[state.bgColour], surface);
    }

    Lemon::Graphics::DrawRect(curPos.x * 8, curPos.y * fontHeight + (fontHeight / 4 * 3), 8, fontHeight / 4,
                              colours[0x7] /* Grey */, surface);
...
void DoAnsiSGR() {
    ...
    if (r < 30) {
        ...
        if (strchr(escBuf, ';') && *(strchr(escBuf, ';') + 1) == '5' &&
            strchr(strchr(escBuf, ';') + 1,
                   ';') /* Check if 2 arguments are given */) { // Next argument should be '5' for 256 colours
    ...
    case ANSI_CSI_EL: {
        ...
        case 1: // Clear from cursor to beginning of line
            if (bufferOffset + curPos.y < static_cast<int>(buffer.size())) {
                if (curPos.x < static_cast<int>(buffer[bufferOffset + curPos.y].size())) {
                    buffer[bufferOffset + curPos.y].erase(buffer[bufferOffset + curPos.y].begin(),
                                                          buffer[bufferOffset + curPos.y].begin() + curPos.x);
                } else {
        ...
        default:
            buffer[bufferOffset + curPos.y].erase(buffer[bufferOffset + curPos.y].begin() + curPos.x,
                                                  buffer[bufferOffset + curPos.y].end());
            break;
        }
        break;
    }
...
int main(int argc, char** argv) {
    ...
    for (;;) {
...

        {
            ...
            if (shouldResize) {
                resizeBounds.x = static_cast<int>(ceil(resizeBounds.x / 8.0)) * 8;
                resizeBounds.y = static_cast<int>(ceil(resizeBounds.y / static_cast<double>(terminalFont->lineHeight))) * terminalFont->lineHeight;

                window->Resize(resizeBounds);
