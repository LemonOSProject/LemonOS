...
void WindowServer::OnPeerDisconnect(handle_t) { throw std::runtime_error("WindowServer has disconnected!"); }
...
void WindowServer::OnThemeUpdate(handle_t, const std::string& name) { (void)name; }

void WindowServer::OnPing(handle_t, int64_t windowID) { Pong(windowID); }
} // namespace Lemon