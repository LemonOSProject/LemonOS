...
/////////////////////////////
inline le_handle_t CreateService(const char* name) { return syscall(SYS_CREATE_SERVICE, name); }
...
///
/// \param service (le_handle_t) Handle ID of the service hosting the interface
...
/////////////////////////////
inline le_handle_t CreateInterface(le_handle_t svc, const char* name, uint16_t msgSize) {
...
///
/// \param interface (le_handle_t) Handle ID of the interface
///
/// \return Handle ID of endpoint on success, 0 when no pending connections, negative error code on failure
/////////////////////////////
inline le_handle_t InterfaceAccept(le_handle_t interface) { return syscall(SYS_INTERFACE_ACCEPT, interface); }
...
/////////////////////////////
inline le_handle_t InterfaceConnect(const char* path) { return syscall(SYS_INTERFACE_CONNECT, path); }
...
///
/// \param endpoint (le_handle_t) Handle ID of specified endpoint
...
/////////////////////////////
__attribute__((always_inline)) inline long EndpointQueue(le_handle_t endpoint, uint64_t id, uint16_t size,
                                                         uintptr_t data) {
...
/////////////////////////////
template <typename T>
__attribute__((always_inline)) inline long EndpointQueue(le_handle_t endpoint, uint64_t id, const T& data) {
...
/////////////////////////////
__attribute__((always_inline)) inline long EndpointDequeue(le_handle_t endpoint, uint64_t* id, uint16_t* size,
                                                           uint8_t* data) {
...
/// \return 0 on success, negative error code on failure
/////////////////////////////
__attribute__((always_inline)) inline long EndpointCall(le_handle_t endpoint, uint64_t id, uintptr_t data, uint64_t rID,
                                                        uintptr_t rData, uint16_t* size) {
...
/// \return 0 on success, negative error code on failure
/////////////////////////////
__attribute__((always_inline)) inline long EndpointInfo(le_handle_t endp, LemonEndpointInfo& info) {
